...
const PollForm = () => {
        ...
        fetch(`${import.meta.env.VITE_BACKEND_URL}/api/polls/`, {
            ...
            .then((data) => {
                if (data.status === "success") {
                    ...
                    if (data.poll) {
                        setPollsData((prev) => prev.map((poll) => (poll.id === pollId ? data.poll : poll)));
                    } else {
    ...
    const handleDeletePoll = async (pollId) => {
        setIsDeleting(true);
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/delete-poll/${pollId}/`, {
                method: "DELETE",
                headers: {
                    Authorization: `Bearer ${JSON.parse(localStorage.getItem("hsapss_tokens"))?.access_token}`,
                },
    ...
    // Check if user can create polls (superadmin, regionadmin or karyakarta)
    const canCreatePolls = user && (user.user_type === "superadmin" || user.user_type === "regionadmin" || user.user_type === "karyakarta");
        ...
        <PhotoProvider>
            ...
            {deleteConfirmModal && (
                <div className="fixed top-0 left-0 w-screen h-screen z-50 bg-black/75 backdrop-blur-sm animate-fadeIn">
                    <div className="fixed top-1/2 left-1/2 w-fit max-w-md transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl animate-slideUp p-6">
                        <div className="flex flex-col items-center gap-4">
                            ...
                            <p className="text-gray-600 text-center">
                                Are you sure you want to delete this poll? This action cannot be undone and will remove all votes and data associated with it.
                            </p>
                            <div className="flex gap-3 w-full mt-4">
                                <button
                                    onClick={() => setDeleteConfirmModal(null)}
                                    disabled={isDeleting}
                                    className="flex-1 px-4 py-2.5 bg-white hover:bg-gray-100 border-2 border-gray-300 text-gray-700 rounded-lg transition-all font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    Cancel
                                </button>
                                <button
                                    onClick={() => handleDeletePoll(deleteConfirmModal)}
                                    disabled={isDeleting}
                                    className="flex-1 px-4 py-2.5 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white rounded-lg transition-all font-bold shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                                >
                                    {isDeleting ? (
                                        <>
            ...
            {pollsData.length > 0 && (
                <div className="p-3">
                    <div className="flex justify-between items-center mb-2">
                        ...
                        {canCreatePolls && (
                            <button type="button" className="px-4 py-2 bg-primary-700 hover:bg-primary-800 text-white rounded-lg transition-all text-sm font-medium" onClick={() => setShowModal(true)}>
                                Create Poll
                            </button>
                        )}
                    </div>
                    ...
                    <div className="flex gap-3 w-full overflow-x-auto pb-2">
                        {pollsData?.map((pollData, index) => {
                            ...
                            return (
                                <div key={index} className="w-96 flex-none bg-white rounded-lg shadow-md p-3 mt-3">

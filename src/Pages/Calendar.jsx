					...
					<div className="w-full max-w-7xl mx-auto lg:px-8">
						<div className="grid grid-cols-12 lg:gap-8 max-w-4xl mx-auto lg:max-w-full">
							<div className="col-span-12 lg:col-span-5">
								<h2 className="text-3xl leading-tight text-primary-600 mb-3">Upcoming Events</h2>
										...
										<h5 className="text-4xl leading-8 text-primary-600 font-haspss flex items-center justify-center gap-3">
											...
											{selectedMonth - 1 === -1 ? (
												<span className="text-xl font-bold whitespace-nowrap">
													( {mandirEvents?.[selectedYear-1]?.[monthNames[11]]?.monthNameGuj}-{mandirEvents?.[selectedYear]?.[monthNames[selectedMonth]]?.monthNameGuj} )
												</span>
											):(
												<span className="text-xl font-bold whitespace-nowrap">
								...
								<div className="border border-secondary-500/50 rounded-xl shadow ">
									...
									<div className="grid grid-cols-7 rounded-b-xl">
										...
										{[...Array(monthDays).keys()].map((day, index) => {
											...
											if (dayData?.iconURL === "" || dayData?.iconURL === undefined) {
												return (
												...
												return (
													<div key={index} title={dayData?.events?.[0]?.eventTitleGuj ? dayData?.events?.[0]?.eventTitleGuj : dayData?.pakshaGuj + " " + dayData?.tithiGuj} className={"group flex flex-col items-center justify-start lg:aspect-square max-lg:min-h-[60px] pt-1 pb-2 overflow-hidden bg-white relative border-secondary-500/50 transition-all duration-300 hover:bg-secondary-50 cursor-pointer " + ((startingDay + day + 1) % 7 === 0 ? " " : "border-r ") + (totalDays - 6 > startingDay + day + 1 ? "border-b" : (startingDay + day + 1) % 7 === 1 ? " rounded-bl-xl" : "")}>
														<img src={dayData?.iconURL} className="w-10 h-10 mix-blend-multiply" alt={dayData?.iconURL} />
														<span className="text-[0.7rem] text-primary-600 text-center line-clamp-2 w-full">{dayData?.events?.[0]?.eventTitleGuj ? dayData?.events?.[0]?.eventTitleGuj : dayData?.pakshaGuj + " " + dayData?.tithiGuj}</span>
														{events.map((event, index2) => {
															if (event?.day === day + 1 && event?.month - 1 === selectedMonth && event?.year === selectedYear) {
																return (
																	<div key={index2} className={`absolute  bottom-1 left-1 p-[0.3rem] group-hover:px-2.5 h-5 group-hover:h-fit w-5 overflow-hidden group-hover:w-fit rounded group-hover:z-50 bg-${event.color}-100 lg:bg-${event.color}-50 border-${event.color}-500 border group-hover:shadow-lg`}>

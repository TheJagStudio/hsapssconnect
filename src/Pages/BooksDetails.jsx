                ...
                <div className="md:flex gap-6">
                    ...
                    <div className="md:w-1/2 bg-white rounded-lg shadow-md overflow-hidden mb-6 md:mb-0">
                        ...
                        <div className="p-6 flex justify-center">
                            <img src={import.meta.env.VITE_BACKEND_URL + book?.poster} alt={book?.title} className="max-w-full max-h-[300px] md:max-h-[400px] object-contain rounded-lg shadow-md" />
                        </div>
                        ...
                        <div className="p-6 pt-0">
                            ...
                            <div className="bg-white shadow-inner p-4 md:p-5 rounded-lg mb-6 border border-primary-100">
                                ...
                                <p className="text-primary-600 leading-relaxed">{showFullDescription ? book?.description : truncateDescription(book?.description)}</p>
                                {book?.description && book.description.split(" ").length > 40 && (
                                    <button onClick={() => setShowFullDescription(!showFullDescription)} className="px-3 py-1 text-center rounded-full w-full max-w-fit h-fit mx-auto text-nowrap border-primary-600 border-2 transition-all bg-gradient-to-tr from-primary-600 to-primary-500 text-white">
                    ...
                    <div className="md:w-1/2 bg-white rounded-lg shadow-md p-6 md:overflow-auto">
                        <h3 className="text-xl font-bold mb-4 text-primary-800">{book?.isPdf ? "Read Book" : "Contents"}</h3>

                        {book?.isPdf ? (
                            // PDF Viewer for PDF books
                            <div className="h-[70vh] w-full">
                                {book?.pdf ? (
                                    <PDFViewer pdfUrl={import.meta.env.VITE_BACKEND_URL + book.pdf} bookId={book?.id} />
                                ) : (
                                    <div className="text-center py-8">
                                        <p className="text-primary-500 italic">PDF file not available</p>
                                    </div>
                                )}
                            </div>
                        ) : book?.sections && book.sections.length > 0 ? (
                            // Traditional chapter list for non-PDF books
                            <div className="space-y-6">
                                {book.sections.map((section, index) => (
                                    <div key={index} className="bg-white p-4 rounded-lg shadow-inner border border-primary-100">
                                        <button onClick={() => toggleSection(index)} className="w-full flex items-center justify-between text-left">
                                            <h4 className={`font-bold w-full text-primary-700 pb-2 ${openSection === index ? "border-b border-primary-200" : ""}`}>{section?.title}</h4>
                                            <svg className={`w-5 h-5 transform transition-transform duration-200 ${openSection === index ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            {section?.chapters && section?.chapters.length > 0 ? (
                                                <ul className="space-y-2">
                                                    {section?.chapters.map((chapter, idx) => (
                                                        <li key={idx} onClick={() => navigate(`/book/${urlId}/${chapter.urlId}`)} className="hover:bg-primary-50 transition-colors duration-200 p-3 rounded-lg cursor-pointer">
                                                            <div className="flex items-center">
                                                                <span className="flex-shrink-0 w-7 h-7 rounded-lg md:rounded-full bg-primary-100 text-primary-700 font-medium flex items-center justify-center mr-3">{idx + 1}</span>
                                                                <span className="text-primary-700 truncate">{chapter?.title}</span>
                                                            </div>
                                                        </li>
                                                    ))}
                                                </ul>
